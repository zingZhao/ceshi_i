...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //收藏
    UITableViewRowAction *noLoveAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"加入黑名单" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        //事件
        NSLog(@"加入黑名单");
        //	将6001加入黑名单
        
        [self tableJionBlackListAtPath:indexPath];
        }];
    //不开心
...
- (void)tableJionBlackListAtPath:(NSIndexPath *)indexPath{
    
    EaseConversationModel *model = [self.dataArray objectAtIndex:indexPath.row];

    EMError *error = [[EMClient sharedClient].contactManager addUserToBlackList:model.conversation.conversationId relationshipBoth:YES];
    if (!error) {
        NSLog(@"发送成功");
...
- (void)tableDelegateCellAtIndexPath:(NSIndexPath *)indexPath{
    EaseConversationModel *model = [self.dataArray objectAtIndex:indexPath.row];
