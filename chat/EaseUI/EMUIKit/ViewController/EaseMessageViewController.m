...
- (void)viewDidLoad {
    ...
    self.chatToolbar = [[EaseChatToolbar alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - chatbarHeight , self.view.frame.size.width, chatbarHeight) type:barType];
...
- (void)_imageMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if ([imageBody type] == EMMessageBodyTypeImage) {
        if (imageBody.thumbnailDownloadStatus == EMDownloadStatusSuccessed) {
            ...
            [weakSelf showHudInView:weakSelf.view hint:@"正在加载"];
            [[EMClient sharedClient].chatManager asyncDownloadMessageAttachments:model.message progress:nil completion:^(EMMessage *message, EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    //send the acknowledgement
...
{
    message.ext = @{@"nickname":[NSString stringWithFormat:@"%@(%@)",[[NSUserDefaults standardUserDefaults]objectForKey:@"nick"],[[NSUserDefaults standardUserDefaults]objectForKey:@"name"]]};
    
    [[NSNotificationCenter defaultCenter]postNotificationName:@"reLisy" object:nil];
