...
#define heiFont 15
#define hei_left 50
@interface GroupListViewController ()< EMGroupManagerDelegate, SRRefreshDelegate,UITableViewDelegate,UITableViewDataSource>
{
...
- (void)viewDidLoad
{
    ...
    heiArray = [[NSMutableArray alloc]init];
...
//    [self.navigationItem setRightBarButtonItems:@[createGroupItem, publicItem]];
    ...
    UIButton *detailButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 17, 85, 35)];
    [detailButton setTitle:@"查看黑名单"  forState:UIControlStateNormal];
    [detailButton addTarget:self action:@selector(showBlickList) forControlEvents:UIControlEventTouchUpInside];
    ...
    if(_dataSource.count == 0 && re == 0){
        [self setSlimeView:self.slimeView];
...
{
    EMGroup *group = [self.dataSource objectAtIndex:indexPath.row];
    CGFloat hei = [self sizeOfStr:group.description andFont:[UIFont systemFontOfSize:heiFont] andMaxSize:CGSizeMake(DEF_SCREEN_WIDTH - hei_left - 20, MAXFLOAT) andLineBreakMode:NSLineBreakByClipping].height;
    [heiArray addObject:[NSString stringWithFormat:@"%ld",hei]];
    if(hei < 10){
        return 50;
    }
    return 40 + hei;
