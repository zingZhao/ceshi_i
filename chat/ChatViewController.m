...
@property (nonatomic,strong)EMGroup * group_my;
...
//        EMError *error = nil;
//        EMGroup *group = [[EMClient sharedClient].groupManager fetchGroupInfo:self.cha includeMembersList:YES error:&error];
...
//    });
    ...
    self.tableView.frame = CGRectMake(self.tableView.frame.origin.x, self.tableView.frame.origin.y, self.tableView.frame.size.width, self.tableView.frame.size.height + 64);
    //self.chatToolbar.frame = CGRectMake(self.chatToolbar.frame.origin.x, self.chatToolbar.frame.origin.y + 50, self.chatToolbar.frame.size.width, self.chatToolbar.frame.size.height);
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(reloadData) name:@"zkb_delete" object:nil];
}
...
- (void)_setupBarButtonItem
{
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        ...
        [detailButton setImage:[UIImage imageNamed:@"group_right"] forState:UIControlStateNormal];
...
- (void)showGroupDetailAction
{
    ...
    if (1) {
        ...
        //detailController.group_my = self.group_my;
...
- (void)deleteAllMessages:(id)sender
{
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"是否清空所有聊天记录" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alertView show];
    }
}
